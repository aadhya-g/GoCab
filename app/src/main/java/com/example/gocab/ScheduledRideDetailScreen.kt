...
fun ScheduledRideDetailScreen(
    ...
    Scaffold(
        ...
        Box(
            ...
            val ride = rideData ?: run {
                Box(Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                    Text("Failed to load ride details", color = Color.White)
                }
                return@Scaffold
            }

            // ✅ CURRENT USER
            val currentStudent = studentsList.find {
                it.optString("S_email_id").equals(userEmail, true)
            }

            val userStatus = currentStudent?.optString("Ride_status") ?: ""
            val rideStatus = ride.optString("R_status")

            val showCancelButton =
                userStatus.equals("Accepted", true) &&
                        !rideStatus.equals("Started", true) &&
                        !rideStatus.equals("Completed", true)
            Column(
                modifier = Modifier
                    .fillMaxSize()
                    .padding(12.dp)
                ...
                Card(
                    ...
                    Column(
                       ...
                       // Spacer(modifier = Modifier.height(40.dp))
                        ...
                        if (userStatus.equals("Cancelled", true)) {
                            Text(
                                "You cancelled your seat",
                                color = Color.Red,
                                fontWeight = FontWeight.Bold
                            )
                        }
                        ...
                        if (rideStatus.equals("Cancelled", true)) {
                            Text(
                ...
                AlertDialog(
                    ...
                    confirmButton = {
                        Button(onClick = {
                            ...
                            cancelRideApi(context, rideId, userEmail) {
                                fetchFullRideDetails(context, rideId) { ride, students ->
                                    rideData = ride
                                    studentsList = students
                                }
...
fun fetchFullRideDetails(
    ...
    val request = JsonObjectRequest(
        ...
        { response ->
            val ride = response.optJSONObject("ride") ?: JSONObject()
            val studentsArray = response.optJSONArray("students")

            val studentsList = mutableListOf<JSONObject>()
...
fun cancelRideApi(
    ...
    val url = "http://10.77.144.204:5000/api/cancel-seat"
...
//            // ✅ FIND CURRENT USER
//            val currentStudent = studentsList.find {
//                it.optString("S_email_id") == userEmail
//            }
//
//            val userStatus = currentStudent?.optString("Ride_status") ?: ""
//            val rideStatus = ride.optString("R_status")
...
//                            fontWeight = FontWeight.Bold)
//
//                        DetailRoww("Name - ${ride.optString("D_name")}")
//                        DetailRoww("Phone - ${ride.optString("D_phone_no")}")
//
//                        // CAR
//                        Text("Car Details",
//                            fontSize = 19.sp,
//                            fontWeight = FontWeight.Bold)
//
//                        DetailRoww("Car - ${ride.optString("C_name")}")
//                        DetailRoww("Number - ${ride.optString("C_number")}")
//
//                        // STUDENTS
...
//                        studentsList.forEach { student ->
//
//                            val status = student.optString("Ride_status")
//
//                            // ✅ ONLY SHOW ACCEPTED STUDENTS
//                            if (status !="Cancelled") {
//
//                                val pickup = student.optString("student_pickup", "N/A")
//                                val drop = student.optString("student_drop", "N/A")
...
//    onResult: (JSONObject?, List<JSONObject>) -> Unit
//) {
//    val url = "http://10.142.248.136:5000/api/ride/full-details/$rideId"
//
//    val request = JsonObjectRequest(
//        Request.Method.GET, url, null,
//        { response ->
...
//    onSuccess: () -> Unit
//) {
//    val url = "http://10.142.248.136:5000/api/cancel-seat"
//
//    val json = JSONObject()
//    json.put("rideId", rideId)
//    json.put("email", email)
//
//    println("🚀 SENDING CANCEL REQUEST: $json")   // 🔥 ADD THIS
//
...
//        json,
//        { response ->
//            println("✅ CANCEL SUCCESS: $response")   // 🔥 ADD THIS
//            Toast.makeText(context, "Ride cancelled", Toast.LENGTH_SHORT).show()
//            onSuccess()
//        },
//        { error ->
...
//
//            println("❌ CANCEL ERROR: ${error.message}")   // 🔥 ADD THIS
//            println("❌ STATUS CODE: ${error.networkResponse?.statusCode}")  // 🔥 ADD
//            println("❌ DATA: ${error.networkResponse?.data?.decodeToString()}") // 🔥 ADD
//
//            Toast.makeText(context, "Cancel failed", Toast.LENGTH_SHORT).show()
//        }
//    ) {}
